...
{
    public class SendOrderDetail : IModule
    {
        //初始化标记
        bool isInit = false;
        //停止执行标记
        ...
        {
            baseModel = OAB.InitBase<SendOrderDetail>();
        }
        ...
        public void Init()
        {
            ...
            {
                //读取配置
                serviceConfig = OAB.InitService<SendOrderDetail>();
        ...
        {
            /***********推送子订单详情***************/
            string _msg = SendOrderDetailToDemandware();
            /***************************************/
        ...
        /// 推送订单详情到Demandware
        /// </summary>
        private string SendOrderDetailToDemandware()
        {
            List<string> _msg = new List<string>();
            FileLogHelper.WriteLog($"Start to send OrderDetail to Demandware.", baseModel.ThreadName);
            //默认读取近25分钟内的变更订单
            DateTime _beginTime = DateTime.Now.AddMinutes(-25);
            ...
            foreach (var api in MallAPIs)
            {
                ...
                if (_result != null)
                {
                    ...
                    {
                        //记录结果
                        _msg.Add($"{api.StoreName()},OrderDetail Send Successfull,Time:{_beginTime.ToString("yyyy-MM-dd HH:mm:ss")}-{_endTime.ToString("yyyy-MM-dd HH:mm:ss")},Total Record:{_result.ResultData.Count},Success Record:{_result.ResultData.Where(p => p.Result).Count()},Fail Record:{_result.ResultData.Where(p => !p.Result).Count()},File Name:{_result.FileName}.");
                    }
                    else
                    {
                        //记录结果
                        _msg.Add($"{api.StoreName()},Time:{_beginTime.ToString("yyyy-MM-dd HH:mm:ss")}-{_endTime.ToString("yyyy-MM-dd HH:mm:ss")},There is no OrderDetail need to be pushed.");
                    }
                }
            }
