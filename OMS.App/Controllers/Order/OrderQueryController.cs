            ...
            using (var db = new ebEntities())
            {
                ...
                switch (_time_type)
                {
                    ...
                    case 2:
                        if (!string.IsNullOrEmpty(_time1))
                        {
                            _sqlWhere.Add(new EntityRepository.SqlQueryCondition() { Condition = "datediff(second, o.PaymentDate, {0}) <= 0", Param = VariableHelper.SaferequestTime(_time1) });
                        }

                        if (!string.IsNullOrEmpty(_time2))
                        {
                            _sqlWhere.Add(new EntityRepository.SqlQueryCondition() { Condition = "datediff(second, o.PaymentDate, {0}) >= 0", Param = VariableHelper.SaferequestTime(_time2) });
                        }
                        break;
                    case 3:
                        if (!string.IsNullOrEmpty(_time1))
                        {
                            _sqlWhere.Add(new EntityRepository.SqlQueryCondition() { Condition = "datediff(second, o.CompleteDate, {0}) <= 0", Param = VariableHelper.SaferequestTime(_time1) });
                        }

                        if (!string.IsNullOrEmpty(_time2))
                        {
                            _sqlWhere.Add(new EntityRepository.SqlQueryCondition() { Condition = "datediff(second, o.CompleteDate, {0}) >= 0", Param = VariableHelper.SaferequestTime(_time2) });
                        }
                        break;
                    default:
                        break;
