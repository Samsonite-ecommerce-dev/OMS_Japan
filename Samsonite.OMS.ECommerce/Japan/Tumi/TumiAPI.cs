...
namespace Samsonite.OMS.ECommerce.Japan.Tumi
{
    public class TumiAPI : ECommerceBase
    {
        ...
        public CommonResult<DeliveryResult> GetTrackingNumbers(List<View_OrderDetail> objOrderDetail_List = null)
        {
            ...
            using (ebEntities db = new ebEntities())
            {
                ...
                if (objOrderDetail_List == null)
                {
                    ...
                    //2.主级子订单
                    //3.最近90天的订单
                    objOrderDetail_List = db.Database.SqlQuery<View_OrderDetail>("select * from View_OrderDetail where MallSapCode={0} and ProductStatus={1} and IsStop=0 and (IsSet=0 or (IsSet=1 and IsSetOrigin=0)) and ParentSubOrderNo='' and IsExchangeNew=0 and IsError=0 and IsDelete=0 and isnull((select PushCount from ECommercePushRecord where PushType={2} and RelatedId=View_OrderDetail.DetailID),0)<{3} and datediff(day,OrderTime,{4})<=0",
                    this.MallSapCode, (int)ProductStatus.Pending, (int)ECommercePushType.RequireTrackingCode, TumiConfig.maxPushCount, _time).ToList();
                }
                //默认快递公司
                ...
                foreach (var _d in objOrderDetail_List)
                {
                    try
                    {
                        ...
                        if (Convert.ToBoolean(_rsp[0]))
                        {
                            ...
                            {
                                //获取次级产品子订单
                                var _sonTmps = db.View_OrderDetail.Where(p => p.OrderNo == _d.OrderNo && p.ParentSubOrderNo == _d.SubOrderNo).ToList();
                                if (_sonTmps.Count > 0)
                                {
                                    foreach (var _sons in _sonTmps)
                                    {
                                        //文档赋值
                                        List<DeliverysDocument> _d_doc = db.DeliverysDocument.Where(p => p.OrderNo == _d.OrderNo && p.SubOrderNo == _d.SubOrderNo).ToList();
                                        foreach (var item in _d_doc)
                                        {
                                            db.DeliverysDocument.Add(new DeliverysDocument()
                                            {
                                                MallSapCode = _sons.MallSapCode,
                                                OrderNo = _sons.OrderNo,
                                                SubOrderNo = _sons.SubOrderNo,
                                                DocumentType = item.DocumentType,
                                                DocumentFile = item.DocumentFile,
                                                CreateTime = DateTime.Now
                                            });
                                        ...
                                        //更新状态
                                        Deliverys _tmpDeliverys = GenericHelper.TCopyValue<Deliverys>(objDeliverys);
                                        _tmpDeliverys.SubOrderNo = _sons.SubOrderNo;
                                        _tmpDeliverys.IsNeedPush = false;
                                        OrderService.OrderStatus_PendingToReceived(_sons, _tmpDeliverys, db);
                                    }
                                }
                            }
        ...
        /// <returns></returns>
        private MonogramDto parseToMonogramItem(string data)
        {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        private GiftCardDto ParseToGiftCardItem(string data)
        {
