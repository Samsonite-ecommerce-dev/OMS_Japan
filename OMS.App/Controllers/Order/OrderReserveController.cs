    ...
    public class OrderReserveController : BaseController
    {
        ...
        public JsonResult Index_Message()
        {
            ...
            using (var db = new DynamicRepository())
            {
                ...
                _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "o.MallSapCode in (select item from strToIntTable('" + string.Join(",", _UserMalls) + "',','))", Param = null });

                if (!string.IsNullOrEmpty(_time1))
                {
                    _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "datediff(day,od.ReservationDate,{0})<=0", Param = VariableHelper.SaferequestTime(_time1) });
                }

                if (!string.IsNullOrEmpty(_time2))
                {
                    _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "datediff(day,od.ReservationDate,{0})>=0", Param = VariableHelper.SaferequestTime(_time2) });
                }
                ...
                if (!string.IsNullOrEmpty(_product_status))
                {
                    ...
                    _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "od.[Status] in (" + _product_status + ")", Param = null });
                }

                //预售订单
                _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "od.IsReservation={0}", Param = 1 });
                //过滤套装主订单
                _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "od.IsSetOrigin={0}", Param = 0 });
                //不显示无效的订单
                _SqlWhere.Add(new DynamicRepository.SQLCondition() { Condition = "od.IsDelete={0}", Param = 0 });
                //查询
                var _list = db.GetPage<dynamic>("select od.Id,od.OrderNo,od.SubOrderNo,o.MallName,o.CreateDate,o.PaymentDate,isnull((select Name from Customer where Customer.CustomerNo=o.CustomerNo),'')As CustomerName,r.[Receive],r.ReceiveTel,r.ReceiveAddr,od.SKU,od.ProductName,od.Quantity,od.SellingPrice,od.Status,od.PaymentAmount,od.IsReservation,od.ReservationDate,od.ReservationRemark,od.ShippingStatus,od.ExchangeQuantity,od.IsError from OrderDetail as od inner join [Order] as o on od.OrderNo=o.OrderNo inner join OrderReceive as r on r.SubOrderNo =od.SubOrderNo order by od.Id desc", _SqlWhere, VariableHelper.SaferequestInt(Request.Form["rows"]), VariableHelper.SaferequestInt(Request.Form["page"]));
                //数据解密并脱敏
