...
{
    public class SendInventory : IModule
    {
        //初始化标记
        bool isInit = false;
        //停止执行标记
        ...
        {
            baseModel = OAB.InitBase<SendInventory>();
        }
        ...
        public void Init()
        {
            ...
            {
                //读取配置
                serviceConfig = OAB.InitService<SendInventory>();
        ...
        private void DoWork()
        {
            ...
            /***********推送库存***************/
            FileLogHelper.WriteLog($"Start to send Inventory to platform.", baseModel.ThreadName);
            ...
            foreach (var api in MallAPIs)
            {
                ...
                {
                    //清空警告库存SKU记录
                    ProductService.DeleteTodayWarnSend(api.StoreSapCode());
                    //记录结果
                    if (!string.IsNullOrEmpty(_result.FileName))
                    {
                        _msg.Add($"{api.StoreName()},Total Record:{_result.ResultData.Count},Success Record:{_result.ResultData.Where(p => p.Result).Count()},Fail Record:{_result.ResultData.Where(p => !p.Result).Count()},File Name:{_result.FileName}.");
                    }
                    else
                    {
                        _msg.Add($"{api.StoreName()},Total Record:{_result.ResultData.Count},Success Record:{_result.ResultData.Where(p => p.Result).Count()},Fail Record:{_result.ResultData.Where(p => !p.Result).Count()}.");
                    }
                }
            }
            /**********************************/
            ...
            //保存结果
            ServiceLogService.Info(serviceConfig.ServiceID, $"{string.Join("<br/>", _msg)}<br/>Next Time:{serviceConfig.NextRunTime.ToString("yyyy-MM-dd HH:mm:ss")}.");
        }
        #endregion
