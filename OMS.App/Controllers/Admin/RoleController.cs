...
namespace OMS.App.Controllers
{
    public class RoleController : BaseController
    {
        ...
        public JsonResult Index_Message()
        {
            ...
            {
                var _lambda = db.SysRole.AsQueryable();

                //搜索条件
                if (!string.IsNullOrEmpty(_keyword))
                {
                    _lambda = _lambda.Where(p => p.RoleName.Contains(_keyword));
                }

                //查询
                var _list = this.BaseEntityRepository.GetPage(VariableHelper.SaferequestInt(Request.Form["page"]), VariableHelper.SaferequestInt(Request.Form["rows"]), _lambda.AsNoTracking(), p => p.SeqNumber, true);
                _result.Data = new
                {
